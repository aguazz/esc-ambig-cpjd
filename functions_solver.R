...
solve_two_stage <- function(params,
  ...
  if (use_nleqslv && requireNamespace("nleqslv", quietly=TRUE)) {
    ...
    out <- nleqslv::nleqslv(theta0, f, method="Broyden",
                            control=list(xtol=1e-10, ftol=1e-12, maxit=200))
    theta <- out$x
    conv  <- out$termcd == 1
    msg   <- out$message
  } else {
    ...
    fit <- optim(theta0, obj, method="BFGS",
                 control=list(reltol=1e-12, maxit=600))
    theta <- fit$par
...
set.seed(1)
sol2 <- solve_two_stage(params, init = list(xL = -1.0, gapU = 1.0))
# sol2 <- solve_two_stage(params, init = smart_init(params))

print(sol2$thresholds)
cat("Gamma from xL:", sol2$gamma, "\n")
cat("Converged:", sol2$status$converged, "-", sol2$status$message, "\n")

diag <- diagnose_solution(sol2, params)

# Plot (with colored verticals as you requested earlier)
xs   <- unlist(sol2$thresholds)[c("xL","x_kappa","x_lambda","xU")]
cols <- c("#B22222","#1E90FF","#8A2BE2","#228B22")
ltys <- c(1,2,2,1)

curve(sol2$functions$H(x),
